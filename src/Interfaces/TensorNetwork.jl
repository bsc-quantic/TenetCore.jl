...
function addtensor!(tn, tensor)
    ...
    prehandle!(tn, PushEffect(tensor))
    addtensor_inner!(tn, tensor)
    posthandle!(tn, PushEffect(tensor))
    return tn
end

prehandle!(tn, @nospecialize(e::PushEffect{<:Tensor})) = prehandle!(tn, e, delegates(TensorNetwork(), tn))
prehandle!(tn, @nospecialize(e::PushEffect{<:Tensor}), ::DelegateTo) = prehandle!(delegate(TensorNetwork(), tn), e)
function prehandle!(tn, @nospecialize(e::PushEffect{T}), ::DontDelegate) where {T<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end

posthandle!(tn, @nospecialize(e::PushEffect{<:Tensor})) = posthandle!(tn, e, delegates(TensorNetwork(), tn))
posthandle!(tn, @nospecialize(e::PushEffect{<:Tensor}), ::DelegateTo) = posthandle!(delegate(TensorNetwork(), tn), e)
function posthandle!(tn, @nospecialize(e::PushEffect{T}), ::DontDelegate) where {T<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end
...
function rmtensor!(tn, tensor)
    ...
    prehandle!(tn, DeleteEffect(tensor))
    rmtensor_inner!(tn, tensor)
    posthandle!(tn, DeleteEffect(tensor))
    return tn
end

prehandle!(tn, @nospecialize(e::DeleteEffect{<:Tensor})) = prehandle!(tn, e, delegates(TensorNetwork(), tn))
prehandle!(tn, @nospecialize(e::DeleteEffect{<:Tensor}), ::DelegateTo) = prehandle!(delegate(TensorNetwork(), tn), e)
function prehandle!(tn, @nospecialize(e::DeleteEffect{T}), ::DontDelegate) where {T<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end

posthandle!(tn, @nospecialize(e::DeleteEffect{<:Tensor})) = posthandle!(tn, e, delegates(TensorNetwork(), tn))
posthandle!(tn, @nospecialize(e::DeleteEffect{<:Tensor}), ::DelegateTo) = posthandle!(delegate(TensorNetwork(), tn), e)
function posthandle!(tn, @nospecialize(e::DeleteEffect{T}), ::DontDelegate) where {T<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end

## `replace_tensor!`
function replace_tensor!(tn, old_tensor, new_tensor)
    prehandle!(tn, ReplaceEffect(old_tensor, new_tensor))
    replace_tensor_inner!(tn, old_tensor, new_tensor)
    posthandle!(tn, ReplaceEffect(old_tensor, new_tensor))
    return tn
end

prehandle!(tn, @nospecialize(e::ReplaceEffect{<:Tensor,<:Tensor})) = prehandle!(tn, e, delegates(TensorNetwork(), tn))
function prehandle!(tn, @nospecialize(e::ReplaceEffect{<:Tensor,<:Tensor}), ::DelegateTo)
    prehandle!(delegate(TensorNetwork(), tn), e)
end
function prehandle!(tn, @nospecialize(e::ReplaceEffect{Told,Tnew}), ::DontDelegate) where {Told<:Tensor,Tnew<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end

posthandle!(tn, @nospecialize(e::ReplaceEffect{<:Tensor,<:Tensor})) = posthandle!(tn, e, delegates(TensorNetwork(), tn))
function posthandle!(tn, @nospecialize(e::ReplaceEffect{<:Tensor,<:Tensor}), ::DelegateTo)
    posthandle!(delegate(TensorNetwork(), tn), e)
end
function posthandle!(tn, @nospecialize(e::ReplaceEffect{Told,Tnew}), ::DontDelegate) where {Told<:Tensor,Tnew<:Tensor}
    throw(MissingEffectHandlerException(tn, e))
end

## `replace_ind!`
function replace_ind!(tn, old_ind, new_ind)
    prehandle!(tn, ReplaceEffect(old_ind, new_ind))
    replace_ind_inner!(tn, old_ind, new_ind)
    posthandle!(tn, ReplaceEffect(old_ind, new_ind))
    return tn
end

prehandle!(tn, @nospecialize(e::ReplaceEffect{<:Index,<:Index})) = prehandle!(tn, e, delegates(TensorNetwork(), tn))
function prehandle!(tn, @nospecialize(e::ReplaceEffect{<:Index,<:Index}), ::DelegateTo)
    prehandle!(delegate(TensorNetwork(), tn), e)
end
function prehandle!(tn, @nospecialize(e::ReplaceEffect{Iold,Inew}), ::DontDelegate) where {Iold<:Index,Inew<:Index}
    throw(MissingEffectHandlerException(tn, e))
end

posthandle!(tn, @nospecialize(e::ReplaceEffect{<:Index,<:Index})) = posthandle!(tn, e, delegates(TensorNetwork(), tn))
function posthandle!(tn, @nospecialize(e::ReplaceEffect{<:Index,<:Index}), ::DelegateTo)
    posthandle!(delegate(TensorNetwork(), tn), e)
end
function posthandle!(tn, @nospecialize(e::ReplaceEffect{Iold,Inew}), ::DontDelegate) where {Iold<:Index,Inew<:Index}
    throw(MissingEffectHandlerException(tn, e))
end
...
function Base.replace!(tn::AbstractTensorNetwork, old_new::Pair{<:Tensor,<:AbstractTensorNetwork})
    ...
    prehandle!(tn, ReplaceEffect(old_new))
    for tensor in tensors(new)
        addtensor_inner!(tn, tensor)
    end
    rmtensor_inner!(tn, old)
    posthandle!(tn, ReplaceEffect(old_new))
...
function Base.replace!(tn::AbstractTensorNetwork, @nospecialize(old_new::Pair{<:Vector{<:Tensor},<:Tensor}))
    ...
    prehandle!(tn, ReplaceEffect(old, new))
    for tensor in old
        rmtensor_inner!(tn, tensor)
    end
    addtensor_inner!(tn, new)
    posthandle!(tn, ReplaceEffect(old, new))
